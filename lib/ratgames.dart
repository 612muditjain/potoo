    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          ...
          dailyCoinsCollected = data['dailyCoinsCollected'];
        });
  ...
  Future<void> convertCoins() async {
    try {
      var response = await http.post(
        ...
        headers: {'Content-Type': 'application/json'},
        body: jsonEncode({'userId': widget.userId, 'coins': totalPoints}),
      );
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          ...
          totalPoints = 0; // Reset points after conversion
        });
        print('Coins converted successfully.');
        await fetchUserDetails();
      } else {
        var errorMessage = jsonDecode(response.body)['message'] ?? 'Failed to convert coins.';
        print('Failed to convert coins: $errorMessage');
      ...
      body: Center(
        child: Column(
          ...
          children: [
            ...
            GestureDetector(
              onTap: () async {
                if (dailyCoinsCollected < 1500) {
                  await collectCoins(2);
                  await fetchUserDetails();
                } else {
